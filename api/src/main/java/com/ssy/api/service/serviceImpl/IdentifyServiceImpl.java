        ...
        if (photo.size() != 0) {
            photo.stream().forEach(i -> {
                if (i.getUrl().trim() != null ) {
                    ...
                    String s = "";
                    //获取识别结果
                    if (i.getUrl().substring(i.getUrl().lastIndexOf(".") + 1).toLowerCase().equals("jpg") ||
                            i.getUrl().substring(i.getUrl().lastIndexOf(".") + 1).toLowerCase().equals("png") ||
                            i.getUrl().substring(i.getUrl().lastIndexOf(".") + 1).toLowerCase().equals("bmp")) {
...
//                } else {
                        ...
                        identify.setPhotoId(i.getId());
                        identify.setIsDelete(CommonConstant.DELFlag);
                        Identify save = identifyRepository.save(identify);
                        ids.add(save.getPhotoId());
                    }
                }
