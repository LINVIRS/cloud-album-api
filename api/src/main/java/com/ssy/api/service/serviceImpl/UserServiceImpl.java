...
/**
 * @ClassName: UserServiceImpl @Description: TODO @Author: WangLinLIN @Date:
 * 2021/04/06 18:10:00  @Version: V1.0
 */
@Service
public class UserServiceImpl implements UserService {
  @Resource private UserRepository userRepository;
  @Resource private TokenRedisDao tokenRedisDao;
  @Resource private CodeRedisDao codeRedisDao;
  ...
  public RestResult UseRegistered(UserDto userDto) {
    ...
    if (userDto.getVerificationCode().equals(userCode)) {
      ...
      user.setPassword(
          SHA256Util.SHA256(
              userDto.getPassword(), MD5Util.MD5Encode(String.valueOf(random), "UTF-8")));
  ...
  @Override
  public RestResult UserAccountCheck(String account) {
  ...
  @Override
  public RestResult UserPhoneCheck(String PhoneNumber) {
    User userByPhone = userRepository.findUserByPhone(PhoneNumber);
    if (userByPhone != null) {
      // 用户已经存在
      return new RestResultBuilder<>().failure(ResultCode.ACCOUNT_EXIST);
    }
